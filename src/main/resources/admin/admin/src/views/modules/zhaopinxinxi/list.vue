<template>
  <div class="main-content">
    ...
    <el-dialog
      ...
      width="800">
        <div id="luyongrenshuChart" style="width:100%;height:600px;"></div>
...
export default {
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        // zhaopingangwei zhaopingangwei
        //  luyongrenshu
        // xcolumn ycolumn zhaopingangwei luyongrenshu
        var luyongrenshuChart = this.$echarts.init(document.getElementById("luyongrenshuChart"),'macarons');
        this.$http({
            url: `value/zhaopinxinxi/zhaopingangwei/luyongrenshu`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].zhaopingangwei);
                    yAxis.push(res[i].total);
                    pArray.push({
                    value: res[i].total,
                    name: res[i].zhaopingangwei
                    })
                    var option = {};
                    option = {
                        title: {
                    ...
                    // 使用刚指定的配置项和数据显示图表。
                    luyongrenshuChart.setOption(option);
                    //根据窗口的大小变动图表
                    window.onresize = function() {
                        luyongrenshuChart.resize();
                    };
                }
            }
        });
